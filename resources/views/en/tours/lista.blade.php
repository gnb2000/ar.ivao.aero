...
@if(count($tours) != 0)
	@foreach($tours as $tour)
		<div class="row">
			...
			<p style="text-align:justify">Aircrafts: {{$tour->aircrafts}}</p>
			<div class="separacion-tablas"></div>
			<p>
			<a class="btn btn-success btn-sm" href="/?pagina=tours/detalles&tour={{$tour->code}}" role="button"><i class="fa fa-plus-square"></i>&nbsp;&nbsp; <strong>See More</strong></a>  
			<a class="btn btn-default btn-sm" href="https://tours.ivao.aero/cgi-bin/tours.pl?division=AR&tour={{$tour->code}}" target="_blank" role="button"><i class="fa fa-plus-square"></i>&nbsp;&nbsp; Report</a><!--  <a class="btn btn-default btn-sm" href="#" role="button"><i class="fa fa-plane"></i>&nbsp;&nbsp; Reservar ATC</a>--></p>
			</div>
			</div>
			<div class="separacion-tablas"></div>';
	@endforeach
@else
	<div class="row">
	<h2 class="title">There are no tours</h2>
	</div>
