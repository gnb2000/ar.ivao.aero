    ...
    if (count($pilots) != 0)
    {
        ...
        foreach($pilots as $pilot)
        {
            ...
            $ciudadll = utf8_encode($filall['ciudad']);
				...
				echo ''.$pilot[0].'|'.$airline.'|'.substr($pilot[9],2,4).'|' .$pilot[11]. '|'.$nombres.'|'.$ciudads.'|'.$paiss.'|'.$etod.'|'.$salida.'|'.$estadoDEP.'|' .$pilot[13]. '|'.$nombrell.'|'.$ciudadll.'|'.$paisll.'|'.$textETA.'|'.$llegada.'|'.$estadoARR.'|<br>';
