...
<thead class="thead-dark">
    ...
    if (count($pilots) != 0)
    {
        foreach($pilots as $pilot)
        {
            ...
            if(strlen($pilot[23]) <= 3) $pilot[23] = str_pad($pilot[23], 4, '0', STR_PAD_LEFT);
            ...
            $timetod = mktime(substr($pilot[23],0,2), substr($pilot[23],2,2)) + 600;
            $etod = date('H:i', $timetod);
			$timeActual = mktime(date('H'));
			$horaActual = date('H', $timeActual);
			$difdia = (int)$horaActual - (int)(substr($pilot[23],0,2));
			if($difdia < -1) $timetod = mktime(substr($pilot[23],0,2), substr($pilot[23],2,2), date('s'), date('n'), (int)date('j') - 1);
            ...
            if($paisSalida == 'Argentina')
            {
                echo '<tr>
                  <td>' .$etod.'</td>
                  <td><a href="/recursos/estado/'.$pilot[0].'">' .$pilot[0]. '</a></td>
    ...
    if (count($pilots) != 0) 
    {
        foreach($pilots as $pilot)
        {
            $pilot[46] = $pilot[46] == 1 ? '<span class="p-3 badge badge-primary">Rodando</span>' : '<span class="p-3 badge badge-info">En Ruta</span>';
      			...
      			$timeETA = obtenerETA($pilot[23], $pilot[25], $pilot[26]);
